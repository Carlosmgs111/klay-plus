...
export class KnowledgeRetrievalOrchestrator {
     ...
     */
    async findMostSimilar(queryText, minScore = 0.7) {
     ...
     */
    async hasSimilarContent(content, similarityThreshold = 0.9) {
     ...
     */
    async findRelated(semanticUnitId, content, options) {
    ...
    async batchSearch(queries, options) {
        const searchResults = await Promise.all(queries.map((query) => this.search(query, {
            limit: options?.limit,
...
export async function knowledgeRetrievalOrchestratorFactory(policy) {
    const { KnowledgeRetrievalOrchestratorComposer } = await import("./composition/KnowledgeRetrievalOrchestratorComposer.js");
