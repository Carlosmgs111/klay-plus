...
export interface ProfileFormFieldsProps {
  initialValues?: {
    ...
    preparationConfig?: Record<string, any>;
    fragmentationStrategy?: string;
    fragmentationConfig?: Record<string, any>;
    projectionStrategy?: string;
    projectionConfig?: Record<string, any>;
  };
  onSubmit: (values: ProfileFormValues) => Promise<any>;
  submitLabel: string;
  loadingLabel: string;
...
export function ProfileFormFields({
  ...
  const { mode, configStore } = useRuntimeMode();
  ...
  const [preparationStrategy, setPreparationStrategy] = useState(init?.preparationStrategy ?? "basic");
  const [prepNormalizeWhitespace, setPrepNormalizeWhitespace] = useState(init?.preparationConfig?.normalizeWhitespace ?? true);
  const [prepNormalizeEncoding, setPrepNormalizeEncoding] = useState(init?.preparationConfig?.normalizeEncoding ?? true);
  const [prepTrimContent, setPrepTrimContent] = useState(init?.preparationConfig?.trimContent ?? true);

  // --- Fragmentation layer ---
  const [fragOpen, setFragOpen] = useState(true);
  const [fragmentationStrategy, setFragmentationStrategy] = useState(init?.fragmentationStrategy ?? "recursive");
  const [fragChunkSize, setFragChunkSize] = useState(init?.fragmentationConfig?.chunkSize ?? 1000);
  const [fragOverlap, setFragOverlap] = useState(init?.fragmentationConfig?.overlap ?? 100);
  const [fragMaxChunkSize, setFragMaxChunkSize] = useState(init?.fragmentationConfig?.maxChunkSize ?? 1000);
  const [fragMinChunkSize, setFragMinChunkSize] = useState(init?.fragmentationConfig?.minChunkSize ?? 100);

  // --- Projection layer ---
  const [projOpen, setProjOpen] = useState(true);
  const [projectionStrategy, setProjectionStrategy] = useState(init?.projectionStrategy ?? "hash-embedding");
  const [projDimensions, setProjDimensions] = useState(init?.projectionConfig?.dimensions ?? 128);
  const [projBatchSize, setProjBatchSize] = useState(init?.projectionConfig?.batchSize ?? 100);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <LoadingButton type="submit" loading={isLoading} loadingText={loadingLabel} disabled={disabled}>
          {submitLabel}
        </LoadingButton>
        {footer}
      </div>
