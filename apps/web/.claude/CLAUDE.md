...
src/
  ...
  pages/api/           # API route files under pipeline/ and lifecycle/
  components/
  ...
  services/            # KnowledgeService interface + dual implementations
    knowledge-service.ts        # Unified interface (20 methods: pipeline + lifecycle)
    server-knowledge-service.ts # Delegates to /api/* routes via server-http-client
    browser-knowledge-service.ts # Direct @klay/core import + IndexedDB
  ...
  server/              # knowledge-singleton.ts — shared platform instance for server adapter
...
- **RuntimeModeContext** — React context for server/browser mode toggle; initialization logic extracted into `initServerMode()` / `initBrowserMode()` pure async functions; exposes single `KnowledgeService` via `useKnowledgeService()` hook
...
- **KnowledgeService** — unified interface organized by resource namespace (`contexts.*`, `sources.*`, `profiles.*`, `process()`, `search()`) with dual implementations (Server: via ServerHttpClient, Browser: direct `@klay/core` import)
...
- `knowledge-singleton.ts` — lazy singleton via `createKnowledgePlatform()`, exposes `getCoordinator()` returning `KnowledgeCoordinator` directly
